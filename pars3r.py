...
def parse_and_simplify(expression):
    ...
    simplified_expr = simplify_boolean_expression(parsed_expr)
    return simplified_expr

if __name__ == "__main__":
    while True:
        expression = input("Enter the Boolean Expression (or 'quit' to exit): ")
        if expression.lower() == "quit":
            break

        try:
            simplified = parse_and_simplify(expression)
